...
# NotConfigured is imported here as well as used here: it lived in this module first, and
# everything that imports it from caldav keeps working. The palette moved out for the same
# reason in reverse — it belongs to neither transport, so Google's can use it too — and is
# re-exported here so nothing that already imported it has to care.
from calendar_errors import CalendarError, NotConfigured
from palette import PALETTE_RGB, nearest_colour
...
class CalDavError(CalendarError):
    """A CalDAV server said something a person can act on. Never carries the password: the
    messages raised here are stored as `calendars.last_error` and shown in the interface,
    and a credential that leaks into either is a credential in a log file.

    NotConfigured is re-exported from calendar_errors below, so `caldav.NotConfigured`
    keeps working for everything that already imports it from here.
    """
